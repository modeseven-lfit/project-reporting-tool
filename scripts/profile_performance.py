...
def run_with_profiling(config_path: Path, max_workers: int, profile_output: Path = None) -> Dict[str, Any]:
    ...
    if profile_output:
        profile_file = profile_output.parent / f"{profile_output.stem}_workers_{max_workers}{profile_output.suffix}"
...
def run_comparison(config_path: Path, worker_counts: List[int], output_dir: Path) -> None:
    ...
    with open(report_file, 'w') as f:
        ...
        for r in results:
            ...
            f.write(f"| {r['max_workers']:7d} | {r['wall_time']:13.2f} | {r['cpu_time']:12.2f} | {r['cpu_utilization']*100:12.1f} | {speedup:8.2f}x |\n")
        
        f.write("\n## Analysis\n\n")
        
        # Find best performer
        best = min(results, key=lambda x: x["wall_time"])
        f.write(f"**Best Performance**: {best['max_workers']} workers ({best['wall_time']:.2f}s)\n\n")
        ...
        if len(results) > 1:
            ...
            for i, r in enumerate(results[1:], 1):
                ...
                f.write(f"- {r['max_workers']} workers: {efficiency:.1f}% efficient (ideal: {ideal_speedup:.1f}x, actual: {actual_speedup:.2f}x)\n")
    ...
    print(f"{'='*80}\n")
    print(f"{'Workers':>8} | {'Wall Time':>12} | {'CPU Time':>11} | {'CPU Util':>10} | {'Speedup':>8}")
    print(f"{'-'*8}-+-{'-'*12}-+-{'-'*11}-+-{'-'*10}-+-{'-'*8}")
    
    for r in results:
        speedup = baseline / r["wall_time"] if r["wall_time"] > 0 else 0
        print(f"{r['max_workers']:8d} | {r['wall_time']:10.2f}s | {r['cpu_time']:9.2f}s | {r['cpu_utilization']*100:9.1f}% | {speedup:7.2f}x")
