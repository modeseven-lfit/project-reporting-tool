...
def run_benchmark(
    ...
    try:
        ...
        profiler.record_metric("success_rate", 
                              results["successful"] / results["total_repos"] * 100,
                              "%")
...
def compare_to_baseline(
    current_reports: Dict[str, ProfileReport],
    baseline_path: str
):
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run baseline performance benchmarks"
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        "--compare",
        help="Compare to existing baseline file"
    )
    parser.add_argument(
        "--small-only",
        action="store_true",
        help="Run only small benchmark (10 repos)"
    )
    parser.add_argument(
        "--large",
        action="store_true",
        help="Include large benchmark (50 repos)"
    )
    ...
    for name, report in reports.items():
        ...
        print(f"  {name.capitalize()}: {repo_count} repos in {duration:.2f}s ({duration/repo_count:.2f}s per repo)")
