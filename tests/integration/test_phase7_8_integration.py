...
class TestPhase7Phase8Integration:
                ...
                'repositories': [
                    {
                        ...
                        'authors': [
                            {'name': f'Author{k}', 'email': f'author{k}@proj{i}.com', 'commit_count': 10}
                            for k in range(5)
                        ]
                    }
                    for j in range(3)
        ...
        reports_data = [
            {
                'project_name': f'Project {i}',
                'repositories': [
                    {'name': f'repo{j}', 'total_commits': 50}
                    for j in range(i + 1)
                ]
            }
            for i in range(5)
    ...
    def test_adaptive_scaling_with_rendering(self, tmp_path):
        ...
        data_list = [
            {'project_name': f'Project {i}', 'repositories': []}
            for i in range(20)
        ]
...
class TestConcurrencyWithComplexRendering:
    ...
    def test_batch_rendering_with_error_collection(self, tmp_path):
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(render_safe, data, i)
        ...
        data_list = [
            {
                'project_name': f'Project {i}',
                'repositories': [
                    {'name': f'repo{j}', 'total_commits': 50, 'authors': []}
                    for j in range(3)
                ]
            }
            for i in range(5)
...
class TestDataFlow:
    ...
    def test_end_to_end_report_generation(self, tmp_path):
        ...
        (html_dir / 'report.html.j2').write_text(
            '<h1>{{ project.name }}</h1>'
            '<p>Commits: {{ summary.total_commits }}</p>'
        )
                ...
                'repositories': [
                    {
                        ...
                        'total_commits': 100,
                        'authors': [
                            {'name': f'Author{j}', 'email': f'author{j}@example.com', 'commit_count': 10}
                            for j in range(20)
                        ]
                    }
                    for i in range(size)
