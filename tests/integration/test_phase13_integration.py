...
from cli.metrics import MetricsCollector, get_metrics_collector
...
class TestPerformanceMetricsIntegration:
    ...
    def test_metrics_formatting_helpers(self):
        ...
        from cli.metrics import format_duration, format_bytes, format_percentage
...
class TestEdgeCases:
    ...
    def test_nonexistent_config_directory(self, temp_dir):
        ...
        args = parser.parse_args([
            '--project', 'test',
            '--repos-path', str(temp_dir),
            '--config-dir', str(temp_dir / 'nonexistent')
        ])
