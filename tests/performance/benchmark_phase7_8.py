...
from rendering.modern_renderer import ModernReportRenderer as NewModernReportRenderer
...
def benchmark_concurrent_rendering(concurrency_levels: List[int] = [1, 5, 10]) -> Dict[int, float]:
    ...
    print(f"\n{'='*60}")
    print(f"Benchmarking Concurrent Rendering")
    print(f"{'='*60}")
...
def benchmark_adaptive_pool(task_counts: List[int] = [10, 50, 100]) -> Dict[int, float]:
    ...
    print(f"\n{'='*60}")
    print(f"Benchmarking Adaptive Thread Pool")
    print(f"{'='*60}")
...
def print_summary(benchmark_results: Dict[str, Any]) -> None:
    ...
    if "report_generation" in benchmark_results:
        ...
        for format_name, times in benchmark_results["report_generation"].items():
            ...
            print(f"  {format_name:20s}: {avg:7.2f}ms (±{stdev:5.2f}ms) "
