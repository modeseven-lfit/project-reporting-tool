    ...
    for i in range(num_projects):
        ...
        for j in range(2):
            ...
            author = AuthorMetrics(
                author_name=f"Dev {i}" if j == 0 else f"Lead {i}",
                author_email=f"dev{i}@example.com" if j == 0 else f"lead{i}@example.com",
                commit_count=50 + (i % 100),
                lines_added=1000 + (i % 5000),
                lines_deleted=500 + (i % 2000),
                first_commit_date=now - timedelta(days=800),
                last_commit_date=now - timedelta(days=days_since_commit),
            )
        ...
        repo_metric = RepositoryMetrics(
            ...
            first_commit_date=now - timedelta(days=800),
            last_commit_date=now - timedelta(days=(i % 1500)),
        )
...
class TestCollectionPerformance:
    ...
    def test_collection_scaling(self, num_projects, sample_yaml_template, base_config):
        ...
        try:
            ...
            # Warmup
            _ = collector.collect_projects(info_master)

            # Benchmark
            times = []
            for _ in range(3):
                start = time.perf_counter()
                projects = collector.collect_projects(info_master)
...
class TestCacheEffectiveness:
    ...
    def test_cache_speedup(self, sample_yaml_template):
        ...
        try:
            ...
            start = time.perf_counter()
            _projects1 = collector_no_cache.collect_projects(info_master)
            time_no_cache = time.perf_counter() - start

            start = time.perf_counter()
            _projects2 = collector_no_cache.collect_projects(info_master)
            ...
            start = time.perf_counter()
            _projects3 = collector_with_cache.collect_projects(info_master)
            time_with_cache_first = time.perf_counter() - start

            start = time.perf_counter()
            _projects4 = collector_with_cache.collect_projects(info_master)
