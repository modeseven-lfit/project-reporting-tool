...
class TestCachePerformanceThresholds:
    ...
    def test_cache_get_within_threshold(
        ...
        assert result == value
        assert_within_threshold(
            duration, perf_thresholds["cache_get"], "Cache get operation"
        )
    ...
    def test_cache_set_within_threshold(
        ...
        assert_within_threshold(
            duration, perf_thresholds["cache_set"], "Cache set operation"
        )

    def test_cache_cleanup_within_threshold(
        self, temp_cache_dir: Path, mock_cache_entries: List[Dict], perf_thresholds: Dict[str, float]
    ):
    ...
    def test_cache_size_within_limit(
        self, temp_cache_dir: Path, perf_thresholds: Dict[str, float]
    ):
        ...
        assert_memory_within_limit(
            actual_size, max_size_mb, "Cache size", margin=0.2
        )
...
class TestWorkerPoolThresholds:
    ...
    def test_worker_pool_task_execution(
...
class TestBatchOperationThresholds:
    ...
    def test_rate_limit_check_within_threshold(
        self, perf_thresholds: Dict[str, float]
    ):
