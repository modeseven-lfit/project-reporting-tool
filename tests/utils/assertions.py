...
def assert_speedup(
    ...
    if actual_speedup < min_speedup:
        pytest.fail(
            f"Speedup of {actual_speedup:.2f}x is less than "
            f"minimum required {min_speedup:.2f}x"
        )
    ...
    if current != baseline:
        ...
        for key in all_keys:
            if key not in current:
                ...
                differences.append(
                    f"Changed: {key} (baseline: {baseline[key]}, "
                    f"current: {current[key]})"
                )
        
        pytest.fail(
            f"Output has changed unexpectedly:\n" + 
...
def assert_valid_json_file(file_path: Union[str, Path]) -> Dict[str, Any]:
    """
    ...
    for key, expected_value in expected_subset.items():
        if key not in actual_dict:
            ...
            differences.append(
                ...
                f"expected {expected_value}, got {actual_dict[key]}"
            )
    
    if differences:
        msg = message or f"Dictionary subset mismatch:\n" + "\n".join(
...
def assert_datetime_recent(
    ...
    if age_seconds > max_age_seconds:
        msg = message or (
            f"Datetime is {age_seconds:.1f}s old, "
            f"exceeding maximum {max_age_seconds:.1f}s"
        )
        pytest.fail(msg)
...
def assert_raises_with_message(
    ...
    **kwargs
    ...
    if expected_message not in actual_message:
        pytest.fail(
            f"Exception message '{actual_message}' "
            f"doesn't contain '{expected_message}'"
        )
