...
class TestTimeWindowProperties:
    ...
    @settings(max_examples=100)
    def test_time_window_creation_with_valid_inputs_succeeds(
        self, days, name, start, end
    ):
    ...
    def test_time_window_rejects_non_positive_days(
        self, invalid_days, name, start, end
    ):
    ...
    def test_time_window_equality_is_value_based(self, w1: TimeWindow, w2: TimeWindow):
        ...
        if (w1.name == w2.name and w1.days == w2.days and
            w1.start_date == w2.start_date and w1.end_date == w2.end_date):
            assert w1 == w2
        else:
            assert w1 != w2
...
class TestTimeWindowStatsProperties:
    ...
    @settings(max_examples=100)
    def test_stats_addition_is_commutative(
    ...
    @given(valid_time_window_stats())
    @settings(max_examples=100)
    def test_stats_to_dict_includes_contributors_conditionally(
    ...
    @settings(max_examples=50)
    def test_stats_sum_of_list_equals_sequential_addition(
        self, stats_list
    ):
...
class TestTimeWindowIntegrationProperties:
    ...
    @given(
        st.lists(valid_time_window(), min_size=2, max_size=5, unique_by=lambda w: w.name)
    )
    @settings(max_examples=50)
