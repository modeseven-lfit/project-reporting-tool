...
class TestAuthorMetricsAggregationProperties:
    ...
    def test_aggregated_loc_maintains_net_invariant(self, authors):
        ...
        for window in windows:
            total_added = sum(
                author.lines_added.get(window, 0) 
                for author in authors
            )
            total_removed = sum(
                author.lines_removed.get(window, 0) 
                for author in authors
            )
            total_net = sum(
                author.lines_net.get(window, 0) 
                for author in authors
            )
    ...
    @settings(max_examples=100)
    def test_combining_two_authors_increases_or_maintains_totals(
        self, author1, author2
    ):
        """Property: Combining authors increases/maintains totals (never decreases)."""
        for window in ["1y", "90d", "30d"]:
            combined_commits = (
                author1.commits.get(window, 0) + 
                author2.commits.get(window, 0)
            )
    ...
    def test_author_aggregation_order_independence(self, authors):
        ...
        for window in windows:
            totals2[window] = sum(
                author.commits.get(window, 0) 
                for author in reversed_authors
            )
...
class TestRepositoryMetricsAggregationProperties:
    ...
    def test_aggregated_repo_commits_equals_sum(self, repos):
        ...
        for window in windows:
            total_commits = sum(
                repo.commit_counts.get(window, 0) 
                for repo in repos
            )
    ...
    def test_aggregated_repo_loc_maintains_net_invariant(self, repos):
        ...
        for window in windows:
            total_added = sum(
                repo.loc_stats.get(window, {}).get("added", 0) 
                for repo in repos
            )
            total_removed = sum(
                repo.loc_stats.get(window, {}).get("removed", 0) 
                for repo in repos
            )
            total_net = sum(
                repo.loc_stats.get(window, {}).get("net", 0) 
                for repo in repos
            )
    ...
    def test_active_repo_count_bounded_by_total(self, repos):
        ...
        active_repos = sum(
            1 for repo in repos 
            if repo.activity_status in ["current", "active"]
        )
    ...
    def test_repos_with_commits_subset_of_all_repos(self, repos):
        ...
        repos_with_commits = sum(
            1 for repo in repos 
            if repo.has_any_commits
        )
...
class TestCrossEntityAggregationProperties:
    ...
    @settings(max_examples=50)
    def test_filtering_by_domain_reduces_or_maintains_count(
        self, authors, target_domain
    ):
        ...
        filtered = [
            ...
            if author.domain == target_domain
        ]
