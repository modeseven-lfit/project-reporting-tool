...
class TestConfigurationError:
    ...
    def test_configuration_error_custom_suggestion(self):
        ...
        error = ConfigurationError(
            ...
            suggestion="Add the 'project' field to config"
        )
...
class TestArgumentParser:
    ...
    def test_get_verbosity_level(self):
        ...
        from src.cli.arguments import parse_arguments, get_verbosity_level, VerbosityLevel
        import tempfile
    ...
    def test_get_log_level(self):
        ...
        from src.cli.arguments import parse_arguments, get_log_level
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Explicit
            args = parse_arguments(['--project', 'test', '--repos-path', tmpdir, '--log-level', 'ERROR'])
            assert get_log_level(args) == 'ERROR'

    def test_get_output_formats(self):
        """Test getting output formats from arguments."""
        from src.cli.arguments import parse_arguments, get_output_formats, OutputFormat
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # HTML only
            args = parse_arguments(['--project', 'test', '--repos-path', tmpdir, '--output-format', 'html'])
            formats = get_output_formats(args)
            assert formats == [OutputFormat.HTML]

    def test_is_special_mode(self):
        """Test detection of special modes."""
        from src.cli.arguments import parse_arguments, is_special_mode
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Show feature mode
            args = parse_arguments(['--project', 'test', '--repos-path', tmpdir, '--show-feature', 'docker'])
            assert is_special_mode(args) is True
